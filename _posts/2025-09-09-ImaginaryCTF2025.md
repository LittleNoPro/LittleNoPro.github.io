...
$$
m \cdot 2^{i+idx} = q \cdot N + m'
$$

- Nếu $q$ chẵn (LSB oracle trả về là `0`):

$$
m \cdot 2^{i+idx} = 2k \cdot N + m' \\
\Rightarrow \frac{k \cdot N}{2^{i+idx-1}} \le m < \frac{k \cdot N }{2^{i+idx-1}} + \frac{N}{2^{i+idx}} \ \ \ \ \ \ \ , \text{do} \ \ 0 \le m' < N  \\
$$
...
$$
m \cdot 2^{i+idx} = (2k+1) \cdot N + m' \\
\Rightarrow \frac{(2k + 1) \cdot N}{2^{i+idx}} \le m < \frac{(2k + 1) \cdot N}{2^{i+idx}} + \frac{N}{2^{i+idx}} \ \ \ \ \ \ \ , \text{do} \ \ 0 \le m' < N  \\
\Rightarrow \frac{k \cdot N}{2^{i+idx-1}} + \frac{N}{2^{i+idx}} \le m < \frac{(2k + 2) \cdot N}{2^{i+idx}} \\
\Rightarrow \frac{k \cdot N}{2^{i+idx-1}} + \frac{N}{2^{i+idx}} \le m < \frac{(k + 1) \cdot N}{2^{i+idx-1}}  \\
$$

Từ 2 trường hợp trên, dễ dàng nhận thấy khi gửi `chosen_ct = long_to_bytes(c * pow(2**(i+idx), e, N) % N)`, nếu oracle trả về `0` thì giá trị `m` sẽ nằm trong nửa khoảng dưới của $[\frac{k \cdot N}{2^{i+idx-1}}, \frac{(k+1) \cdot N}{2^{i+idx-1}})$, và ngược lại khi oracle trả về `1` thì ta biết `m` sẽ thuộc khoảng nửa trên của $[\frac{k \cdot N}{2^{i+idx-1}}, \frac{(k+1) \cdot N}{2^{i+idx-1}})$.
