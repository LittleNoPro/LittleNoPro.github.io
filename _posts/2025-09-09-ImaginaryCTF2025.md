...
\Rightarrow \frac{(2k + 1) \cdot N}{2^{i+idx}} \le m < \frac{(2k + 2) \cdot N}{2^{i+idx}} \\
\Rightarrow \frac{(2k + 1) \cdot N}{2^{i+idx}} \le m < \frac{(k + 1) \cdot N}{2^{i+idx-1}}  \\
$$

Từ 2 trường hợp trên, dễ dàng nhận thấy nếu oracle trả về `0` thì giá trị `m` sẽ nằm trong nửa khoảng dưới của $[)$
