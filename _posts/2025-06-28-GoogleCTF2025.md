...
tags: [HASH, ChaCha, LWE, Lattice]
published: true
description: "Write-up for GoogleCTF 2025"
---

Vừa qua mình có tham gia giải Google CTF, đây là các bài mà mình đã làm được trong và sau giải :3.
...
```
### Phân tích:
Server trong bài này xây dựng một `Merkle Tree`, và có các thao tác để ta tương tác với cây:
1. `deposit(amount, public_key)`: tăng giá trị của `tree[public_key]` lên một lượng `amount` $\in$ `(0, M]`.
2. `withdraw(amount, signature, proof)`: giảm `tree[public_key]` một lượng `amount` $\in$ `(0, M]` với `public_key` được tính lại từ `signature` và `proof`.
3. `flag`: lấy được `flag` khi ta làm cho giá trị `M >= 10^18`.

`M` ở đây là `player_local_amount`.

### Solution
Trong `Merkle tree`, mỗi node sử dụng công thức hash chung như sau:

$$
bcrypt_N(left \ subtree \ hash \ || \ ::: \ || \ right \ subtree \ hash \ || \ ::: \ || \ bcrypt_V(value))
$$

$bcrypt_N, bcrypt_V$ là hash của `bcrypt` với các `salt` lần lượt là `SALT_FOR_NODE` và `SALT_FOR_VALUE`.

Ta thấy rằng, hàm `hash` trong bài này chỉ lấy 31 bytes cuối sau khi `bcrypt`. Tức là các của chuỗi sau khi `hash` sẽ thỏa mãn re `[./0-9A-Za-z]{31}`.

Một tính chất đặc biệt nữa của `bcrypt` đó là `bcrypt` chỉ mã hóa **72 bytes** đầu tiên của `message` (https://man.openbsd.org/bcrypt). Điều đó tương đương với:
...
Giá trị `hash` của mỗi node chỉ bị ảnh hưởng bởi **4 bytes** đầu tiên của `value`.

Đến đây, ta có thể tìm 2 giá trị:
1. `VALID_VALUE` sẽ là giá trị nằm trong khoảng `[0, 10^9]`.
2. `INVALID_VALUE` là giá trị nằm trong khoảng `[10^18, 2^64]`.

Sao cho 2 giá trị đó sau khi `hash` thì sẽ có chung **4 bytes** đầu tiên. Vì mỗi byte sau khi `hash` chỉ có 64 giá trị, nên số trạng thái kết quả của ta sẽ là $64^4$. Sử dụng nghịch lý `Birthday paradox`, ta chỉ cần lấy $\sqrt{64^4} = 64^2 = 2^{12}$ lần random để tìm được `collision`.

Vì ban đầu ta chỉ dùng `deposit` 1 lần để `set(VALID_VALUE)`, nên `Merkle tree` sẽ chỉ có 1 đường đi duy nhất. Từ đó suy ra, `proof` của ta sẽ chỉ gồm các `EMPTY_NODE_HASH`.

Tóm lại, cách giải quyết bài này như sau:
- Tự tạo một bộ khóa ECDSA để ký.
- Tìm được 2 giá trị `VALID_VALUE` và `INVALID_VALUE` có tính chất như trên và thỏa mãn điều kiện :

$10^9 - VALID \ VALUE + INVALID \ VALUE \ge 10^{18}$.
- Gọi hàm `diposit(VALID_VALUE, public_key)` để tạo 1 node duy nhất trên cây.
- Tạo `proof = INVALID_VALUE + EMPTY_NODE_HASH + EMPTY_NODE_HASH + ...`.
- Dùng khóa ECDSA để ký `proof`.
- Gọi `withdraw(INVALID_VALUE, signature, proof)` đến server.
